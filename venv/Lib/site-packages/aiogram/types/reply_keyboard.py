...
class ReplyKeyboardRemove(base.TelegramObject):
    ...
    and display the default letter-keyboard. By default, custom buttons are displayed until a new keyboard is sent by a bot.
    An exception is made for one-time buttons that are hidden immediately after the user presses a button (see ReplyKeyboardMarkup).
